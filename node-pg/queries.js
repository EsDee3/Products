...
module.exports = {
  ...
  getProduct: async (pid) => {
    ...
    try {
      let { rows } = await client.query(prodQuery);
      client.release();
      return rows[0];
    } catch (err) {
  ...
  getRelated: async (pid) => {
    const relatedQuery = {
      name: `fetch-${pid}-related`,
      text:`SELECT array_agg(related_id)
        FROM related
        WHERE pid=$1
      `,
      values: [pid]
    ...
    try {
      let { rows } = await client.query(relatedQuery);
      client.release();
  ...
  getArrayProducts: async (relatedArray) => {
    const relProdsQuery = {
      ...
      FROM product_fnl pf
      WHERE currentProductId IN (${relatedArray.toString()})
      `
    }
    ...
    try {
      let { rows } = await client.query(relProdsQuery);
      client.release();
